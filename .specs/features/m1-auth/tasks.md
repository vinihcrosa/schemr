...
- [ ] Re-exports `auth as middleware` from `auth.ts`
- [ ] `config.matcher` uses regex that excludes: `_next/static`, `_next/image`, `favicon.ico`, `api/auth`, and common image extensions
...
- [ ] File exports `{ GET, POST }` from `auth.handlers`
...
- [ ] `POST /api/auth/register` with valid `{ email, name?, password }` creates User and returns `201 { id, email }`
- [ ] Zod validates: email format, password minimum 8 characters
- [ ] Returns `400` with field errors when Zod validation fails
- [ ] Returns `409` when email already exists (message: "An account with this email already exists")
- [ ] Password stored as bcrypt hash (cost 12) — never plaintext
- [ ] `userId` is never accepted from request body (only generated by DB)
