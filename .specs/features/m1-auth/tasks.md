...
**Status**: Complete — all gate checks pass
...
- [x] TypeScript reports no errors on `session.user.id` access in server code
...
- [x] TypeScript reports no errors (blocked: awaiting lib/db.ts from infra T4)
- [x] Gate check passes: `npm run lint && npm run test:unit`
...
- [x] No direct calls to `auth()` exist outside this file in application code
- [x] Gate check passes: `npm run lint && npm run test:unit`
...
- [x] Unauthenticated request to `/` is redirected to `/sign-in`
- [x] Request to `/api/auth/signin` passes through without redirect loop
- [x] Request to `/_next/static/**` is NOT intercepted by auth middleware
- [x] Integration test covers: unauthenticated → redirect, static path → no redirect
- [x] Gate check passes: `npm run lint && npm run test:unit && npm run test:integration`
...
- [x] `POST /api/auth/signin` with valid credentials creates a session cookie
- [x] `POST /api/auth/signin` with invalid credentials returns error (no 500)
- [x] `POST /api/auth/signout` destroys the session
- [x] Integration test covers: valid sign-in → session cookie set; invalid sign-in → no session; sign-out → session gone
- [x] Gate check passes: `npm run lint && npm run test:unit && npm run test:integration`
...
- [x] Integration test covers: success → 201 + user in DB; duplicate email → 409; short password → 400
- [x] Gate check passes: `npm run lint && npm run test:unit && npm run test:integration`
...
- [x] Unit test: renders children correctly; applies correct layout structure
- [x] Gate check passes: `npm run lint && npm run test:unit`
...
- [x] Unit test: renders form; shows error on failed sign-in; redirects on success (mock signIn)
- [x] Gate check passes: `npm run lint && npm run test:unit`
...
- [x] Unit test: renders form; shows 409 error; shows 400 field errors; calls signIn on success
- [x] Gate check passes: `npm run lint && npm run test:unit`
...
- [x] Unit test: renders placeholder text
- [x] Gate check passes: `npm run lint && npm run test:unit`
...
- [ ] Gate check passes: `npm run test:e2e` (needs playwright install + running app)
